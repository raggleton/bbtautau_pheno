...
bbtauHist::bbtauHist(string model, double fact) {
  ...
  bbtauhlSub.book("", 100, 0., 150.);
  bbtauhlSub.null();
  a1a1hlSub.book("", 100, 0., 150.);
  a1a1hlSub.null();
  h1a1a1hlSub.book("", 100, 0., 100.);
  h1a1a1hlSub.null();
  a1h1hlSub.book("", 100, 0., 150.);
  a1h1hlSub.null();
...
void bbtauHist::Fill(MyProcess *Mp, MyEvent *Me) {
  ...
  if (Me->bbtau(&bbtauHad, &a1a1Had, &h1a1a1Had, &a1h1Had, 10)) {
    ...
    Me->InvM(&bbHad, -5, -5, 0.5);
    Me->InvM(&tataHad, 15, -15, 1);
    Me->InvM(&tataHad, 15, 15, 0.5);
    Me->InvM(&tataHad, -15, -15, 0.5);
  }
}

void bbtauHist::AddEvent() {
  N++;
}
