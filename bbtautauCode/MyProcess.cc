...
MyProcess::MyProcess(Pythia *p) {
  ...
  for (int i = 0; i < pythia->process.size(); i++) {
    if (abs(pythia->process[i].eta()) > 2.5) {
      pythia->process[i].statusNeg();
    }
  }
  double px = 0;
  double py = 0;
  ...
  for (int i = 0; i < pythia->process.size(); i++) {
    ...
    case 5:
      ...
      if (pythia->process[i].pT() > 15. && pythia->process[i].status() > 0) {
        pT15lep++;
      }
      break;
    case -5:
...
bool MyProcess::Fourb(Hist *b4, Hist *a1a1, Hist *h1a1a1, Hist *a1h1,
    double bres) {
  int bArray[4];
  if (b.size() + bp.size() != 4) {
    return false;
    ...
    for (int i = 0; i < bp.size(); i++) {
      bArray[b.size() + i] = bp[i];
    }
  }
  double Inv4b = m(pythia->process[bArray[0]].p(),
      pythia->process[bArray[1]].p(), pythia->process[bArray[2]].p(),
      pythia->process[bArray[3]].p());
  b4->fill(Inv4b);
  int tempA[2][3] = { { 2, 1, 1 }, { 3, 3, 2 } };
  double Inv1, Inv2;
  for (int i = 1; i < 4; i++) {
    Inv1 = m(pythia->process[bArray[0]].p(), pythia->process[bArray[i]].p());
    Inv2 = m(pythia->process[bArray[tempA[0][i - 1]]].p(),
        pythia->process[bArray[tempA[1][i - 1]]].p());
...
double MyProcess::DRaa() {
  ...
  a1.resize(0);
  for (int i = 0; i < pythia->process.size(); i++) {
    if (pythia->process[i].id() == 36) {
      a1.push_back(i);
    }
  }
  if (a1.size() == 2) {
    return deltaR(a1[0], a1[1]);
  } else {
    return 0;
  }
}
...
bool MyProcess::Zbb(Hist *bbh1, Hist *Za1, double bres) {
  ...
  double Inv2b = m(pythia->process[bArray[0]].p(),
      pythia->process[bArray[1]].p());
  double InvZa1 = m(pythia->process[Zint].p(), pythia->process[bArray[0]].p(),
      pythia->process[bArray[1]].p());
  Za1->fill(InvZa1);
...
void MyProcess::Spec(Hist *h, int sp, double w) {
  ...
  int temp = 0;
  for (int i = 0; i < max; i++) {
    h->fill(pythia->process[I(sp, i)].pT(), w);
  }
  max = this->MaxI(-sp);
  temp = 0;
  for (int i = 0; i < max; i++) {
  ...
  for (int i1 = 0; i1 < max1; i1++) {
    for (int i2 = 0; i2 < max2; i2++) {
      ...
      if (!Same(temp1, temp2, sp1, sp2)) {   //temp1 != temp2  || sp1 != sp2
        h->fill(this->m(pythia->process[temp1].p(), pythia->process[temp2].p()),
            w);
      }
    }
  }
    ...
    for (int i2 = 0; i2 < max2; i2++) {
      for (int i3 = 0; i3 < max3; i3++) {
        ...
        if (!Same(temp1, temp2, sp1, sp2) && !Same(temp1, temp3, sp1, sp3)
          ...
          h->fill(
              this->m(pythia->process[temp1].p(), pythia->process[temp2].p(),
                  pythia->process[temp3].p()), w);
        }
      }
    }
  }
    ...
    for (int i2 = 0; i2 < max2; i2++) {
      for (int i3 = 0; i3 < max3; i3++) {
        for (int i4 = 0; i4 < max4; i4++) {
          ...
          temp4 = this->I(sp4, i4);
          if (!Same(temp1, temp2, sp1, sp2) && !Same(temp1, temp3, sp1, sp3)
              && !Same(temp3, temp4, sp3, sp4) && !Same(temp3, temp2, sp3, sp2)
              && !Same(temp2, temp4, sp2, sp4)
              && !Same(temp3, temp4, sp3, sp4)) {
            h->fill(
                this->m(pythia->process[temp1].p(), pythia->process[temp2].p(),
                    pythia->process[temp3].p(), pythia->process[temp4].p()), w);
          }
        }
      }
    }
...
bool MyProcess::AcceptLepton(int lep) {
  if (pythia->process[lep].pT() < 15.
      || abs(pythia->process[lep].eta()) > 2.5) {
    return false;
  }
  return true;
}
...
double MyProcess::deltaR(int i1, int i2) {
  return sqrt(
      pow(pythia->process[i1].eta() - pythia->process[i2].eta(), 2)
          + phi2(pythia->process[i1].phi() - pythia->process[i2].phi()));
}
