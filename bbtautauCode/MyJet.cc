...
MyJet::MyJet(Pythia *p, double Rparam) {
  ...
  for (int i = 0; i < pythia->event.size(); i++) {
    if (pythia->event[i].isFinal() && IsHadron(pythia->event[i].id())
        && abs(pythia->event[i].eta()) < 2.5) {
      PseudoJet temp(pythia->event[i].px(), pythia->event[i].py(),
          pythia->event[i].pz(), pythia->event[i].e());
      for (unsigned int itau = 0; itau < taub.size(); itau++) {
        if (pythia->event[i].isAncestor(taub[itau])
            && deltaR(i, taub[itau]) < R) {
          temp.set_user_index(Bmes(taub[itau]));
        }
      }
      if (abs(abs(pythia->event[i].id()) - 12) == 1) {
        temp.set_user_index(pythia->event[i].id());
      }
  ...
  for (unsigned int i = 0; i < Jets.size(); i++) {
    ...
    if (Jets[i].perp() >= 20 && Jets[i].user_index() == 0) {
      Jet20.push_back(i);
    }
    if (Jets[i].perp() >= 100 && abs(Jets[i].user_index()) < 10) {
      Jet100.push_back(i);
    }
  }
  for (unsigned int i = 0; i < Jets.size(); i++) {
...
int MyJet::part(int i) {
  return Jets[i].user_index();
}
bool MyJet::lep(int i) {
  if (abs(abs(Jets[i].user_index()) - 12) == 1) {
    return true;
  } else {
    return false;
  }
...
double MyJet::eta(int i) {
  return Jets[i].eta();
}
double MyJet::phi(int i) {
  return Jets[i].phi_std();
}
double MyJet::pT(int i) {
  return Jets[i].perp();
}
double MyJet::e(int i) {
  return Jets[i].e();
}
double MyJet::px(int i) {
  return Jets[i].px();
}
double MyJet::py(int i) {
  return Jets[i].py();
}
Vec4 MyJet::p(int i) {
  return Vec4(Jets[i].px(), Jets[i].py(), Jets[i].pz(), Jets[i].e());
}
vector<int> MyJet::GettauJet() {
  return tauJet;
}
vector<int> MyJet::GettaupJet() {
  return taupJet;
}
vector<int> MyJet::GetbJet() {
  return bJet;
}
vector<int> MyJet::GetbpJet() {
  return bpJet;
}
vector<int> MyJet::GetJet20() {
  return Jet20;
}
vector<int> MyJet::GetJet100() {
  return Jet100;
}
vector<int> MyJet::GetFatbb() {
  return FatbbJet;
}
...
double MyJet::deltaR(PseudoJet J1, int i2) {
  return sqrt(
      pow(J1.eta() - pythia->process[i2].eta(), 2)
          + phi2(J1.phi() - pythia->process[i2].phi()));
}
...
int MyJet::Jettype(PseudoJet *J, ClusterSequence *CS) {
  ...
  for (unsigned int iCo = 0; iCo < tautemp.size(); iCo++) {
    ...
    case 15:
      taumx += tautemp[iCo].px();
      taumy += tautemp[iCo].py();
      break;
    case -15:
      taupx += tautemp[iCo].px();
      taupy += tautemp[iCo].py();
      break;
    case 5:
      bmx += tautemp[iCo].px();
      bmy += tautemp[iCo].py();
  ...
  if (taumx * taumx + taumy * taumy > 0.36 * J->kt2()) {
    ret = 15;
  }
  if (taupx * taupx + taupy * taupy > 0.36 * J->kt2()) {
    ret = -15;
  }
  if (bmx * bmx + bmy * bmy > 0.36 * J->kt2()) {
    ret = 5;
  }
  if (bpx * bpx + bpy * bpy > 0.36 * J->kt2()) {
    ret = -5;
  }
  if (emx * emx + emy * emy > 0.49 * J->kt2()) {
    ret = 11;
  }
  if (epx * epx + epy * epy > 0.49 * J->kt2()) {
    ret = -11;
  }
  if (mumx * mumx + mumy * mumy > 0.49 * J->kt2()) {
    ret = 13;
  }
  if (mupx * mupx + mupy * mupy > 0.49 * J->kt2()) {
    ret = -13;
  }
...
void MyJet::SubJets(ClusterSequence *CS) {
  ...
  for (int i = 0; i < Jets.size(); i++) {
    if (Jets[i].pt() > 30. && Jets[i].m() > 12.) {
      p1 = Jets[i];
      int bNum = 0;
      for (int i1 = 0; i1 < pythia->process.size(); i1++) {
        double DRtemp = deltaR(p1, i1);
        bool closest = true;
        if (abs(pythia->process[i1].id()) == 5 && DRtemp < 1.2
            && abs(pythia->process[i1].eta()) < 2.5) {
          for (int i2 = 0; i2 < Jets.size(); i2++) {
            if (DRtemp > deltaR(Jets[i2], i1) && i != i2) {
              closest = false;
            }
          }
          if (closest)
      ...
      if (mu != 0.0 || y != 1.0) {
        ...
        // cout << "1.3" << flush;
        JetDefinition *JD = new fastjet::JetDefinition(cambridge_algorithm, Rp,
            E_scheme, Best);
        vector < PseudoJet > temp;
        ...
        if (temp.size() >= 3) {
          ...
          if ((abs(Jettype(&temp[0], &clustSeq)) == 5
              || abs(Jettype(&temp[1], &clustSeq)) == 5) && p.m() > 12.) {
            Fat1tag.push_back(p);
          }
          if (abs(Jettype(&temp[0], &clustSeq)) == 5
              && abs(Jettype(&temp[1], &clustSeq)) == 5 && p.m() > 12.) {
...
bool MyJet::SameJet(PseudoJet J1, PseudoJet J2) {
  if (J1.pz() == J2.pz() && J1.px() == J2.px() && J1.py() == J2.py()) {
    return true;
  } else {
    return false;
  }
